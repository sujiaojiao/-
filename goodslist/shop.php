...
<?php
    ...
    $sql="select * from productpp where id='$pp'";
	...
	$ppname=$listInfo["ppname"];  //得到上级品牌的名称
...
<html>
	...
	<body id="userPassword" style="background: #f3f4f5;">
		...
		<div class="top_text text-center">
			...
			<a href="<?php echo $page->pageurl2('attr');?>2" class="show-mode-but <?php if($attr==2) echo "cur";?>">推荐 </a>
		</div>
...
//							     echo "<h3 class=\"bb-info\"><a target=\"_blank\" class=\"bb-info-a\" href='{$webdir}goodslist/show.php?id={$rows["id"]}'>".strLeft($rows["title"],20,"...")."123</a><span class=\"mbb-p-title-a\">盾价:</span><strong class=\"mbb-price\">￥{$rows["price"]}</strong><s>￥{$rows["yprice"]}</s></h3></li>";
//							 }}else{
//							     echo "暂无数据";
//							 }
							 ?>
